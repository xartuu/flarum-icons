...
class EditIconHandler
{
  ...
  public function handle(EditIcon $command)
  {
    ...
    if(isset($data["elementPath"])) $icon->element_path = $data["elementPath"];
    if(isset($data["originalIcon"])) $icon->original_icon = $data["originalIcon"];
    if(isset($data["modifiedIcon"])) $icon->modified_icon = $data["modifiedIcon"];
    if(isset($data["isActive"])) $icon->is_active = (bool)$data["isActive"];
